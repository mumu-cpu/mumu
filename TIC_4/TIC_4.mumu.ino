...
unsigned long veil_millis = millis();	 // tempo veil detection
bool selec_mode_GEN = false;	 // state veil detection
unsigned long tempo_surchauffe = millis();// tempo clio normal
...
bool maz_int = false;
bool mis_veil = false;
int etat_sys = 30;			// init valeur
int etat_sys_prew = 30;
/*	etat system
1 test en cours
...
void loop() {
	...
	if (mis_veil == true || etat_sys == 30)  //					VEILLE SYSTEME
	{
	...
	*/
	 ...
	 *		30	init systeme
	 *		1	test en cours
	 *		2	test en cours ccphnete 0 attente
	 *		3	battlow -20%
	 *		4	alim off
	 *		5	tempo auto off
	 *		6	batt off
...
{
	etat_sys = 8;//generateur en cours
	long gen_rlt = millis();
	gen_debut = millis();
	while (gen_rlt <= gen_tmp_230)
	{
	...
	etat_sys = 21;// sortie generateur 230v
...
void gen_tst()
{
	...
	while (tempo_State_On == true)
	{
		...
		if (CcPhNeTe >= 4 && CcPhNeTe <= 8)	/*	TEST  EN COURS	CcPhNeTe 4-7	*/
		{
			debut = millis();
		}
		else								/*	TEST  EN COURS	CcPhNeTe 0		*/
		{
			...
			{
				etat_sys = 2;// test en cours attente
			}
...
void manu_auto_tempo() {
	//            MANU_AUTO_VEILLE
	if (etat_sys == 30 || etat_sys == 8) // init manu_auto
	{
		tempo_State_On = false;
		return;
	}
...
void veil_mis()
{
	...
	//long veil_millis = millis();
	//long veil_millis_prew = millis();
	long rlt_veil = millis() - veil_millis;
	long interval_veil_1 = 1 * minut;			//	temps arret
	long interval_veil_2 = 5 * sec;			//	temps marche
	//bool selec_mode_GEN = false;
	etat_sys = 20; // veille en cours

	//maz_int = false;	maz_int_f();
	//veil_millis = millis();

	if (selec_mode_GEN == false)
	{
		rlt_veil = millis() - veil_millis;
		if (rlt_veil >= interval_veil_1)
		{
			selec_mode_GEN = true;
			veil_millis = millis();
		}
	...
	if (battOff == false && alim_State == true && SYS_SUR_CHAUFFE == false)
	{
		if (selec_mode_GEN == true)
		{
			getAlim(1);
	...
	while (battOff == true || alim_State == false || SYS_SUR_CHAUFFE == true)
	{
		getBattery();
		temp_lux_systeme();
		init_sys();
		...
		case 7:// SUR_CHAUFFE SYS
			...
			GEN_TEST_stat = false;
			tempo_State_On = false;
			//tempo_surchauffe_rsl = millis() - tempo_surchauffe;
	...
	debut = millis();
	mis_veil = false;
	etat_sys = 23; // sortie veille
}
void sys_veille()
{
	if (etat_sys != 30) // init veille
	{
		etat_sys = 20; // veille en cours
	}
	byte switch_veil;
...
void clio_normal(int sys_defaut)
{
	...
	{
		if (millis() - tempo_surchauffe >= 2 * sec)
		{
			tempo_surchauffe = millis();
			state_sortie = true;
		}
	}
...
{
	long rlt_alim = millis() - curren_millis_alim;
	//Mar_Arr = etat_sys;
	if (etat_sys == 30) // init getAlim
	{
	...
	if (etat_sys == 6 || etat_sys == 7) // 6-batt_off || 7-surchauffe
	{
		//digitalWrite(alim, true);
		digitalWrite(alim, false);
	...
	{
		if (etat_sys != 4) // alim off
		{
			alim_State = true;
		}
		if (etat_sys != 7) // surchauffe
		{
