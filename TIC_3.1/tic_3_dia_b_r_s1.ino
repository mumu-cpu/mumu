...
{
  // int ct1s_state = count1s(ct1s_state);
  int ct05s_ste = ct05s_state;
  unsigned long rlt05s = millis() - cpt05s_millis;
  // if ((compteB == 1 || compteB == 0) && ct1s_state > 2)
  // {
  //   cpt1s_millis = millis();
  // }
  ...
  if (ct05s_sta == true && ct05s_sta1 == false && rlt05s > 100) // && rlt1s < 1000)
  {
    cpt05s_millis = millis();
    ct05s_ste = ct05s_ste + 1;
    ct05s_sta1 = true;
  ...
  {
    ct05s_sta=false;ct05s_sta1=false;
  }
...
int sosNbr23(int compteB) // CcPhNeTe 3
{
  Serial.print("compteB_");
  Serial.println(compteB);
  switch (compteB) //(sosNbr == 23)
...
int ccphnt()
{
  ...
  if (phase == true && neutre == true && terre == true)
  {
    if (curmill - chro >= tempi)
    {
      while (phase == true && neutre == true && terre == true)
      {
        // sosNbr = 234;  	// Cc Ph Ne Te
        // sosNbr = 23;  	  // Ph Ne
        // sosNbr = 34;   	// Ne Te
        // sosNbr = 42;   	// Ph Te
        sosNbr = 55; // Ph Ne Te
        ...
        if (CcPhNeTe == 4)
        {
          compteB = 1; // ccphnt while CcPhNeTe == 4
        }
        clioFlache(sosNbr, compteB);
        CcPhNeTe = 5;
        ...
        else
        {
          if (cp_cl <= 2)
          {
            sosSay = 1;
            sosTmp = 1;
            spot_cl(sosSay, sosTmp); //(sosNbr == 55)
          }
          else
          {
            if (cp_cl <= 3)
...
int contPhNe()
{
  ...
  while (phase == true && neutre == true && terre == false)
  {
    // sosNbr = 234;  // Ph Ne Te
    sosNbr = 23; // Ph Ne
    // sosNbr = 34;   // Ne Te
    // sosNbr = 42;   // Ph Te
    // sosNbr = 0;   // arret flache Bleu Rouge
    // sosNbr=55;   //
    // Serial.println(sosNbr);
    // clioFlache(sosNbr, cp_cl);
    CcPhNeTe = 3;
    phase = phaseF();
    neutre = neutreF();
    terre = terreF();
    ...
    if (compteB <= 1)
    {
      sosSay = 2;
      sosTmp = 2;
      f_b_cl(sosSay); //(sosNbr == 23)
      sosSay = 0;
      sosTmp = 0;
      spot_cl(sosSay, sosTmp); // (sosNbr == 23)
    }
    else
    {
      if (compteB <= 2)
...
int contPhTe()
{
  ...
  if (phase == true && neutre == false && terre == true)
  {
    while (phase == true && neutre == false && terre == true)
    {
      // sosNbr = 234;  // Ph Ne Te
      // sosNbr = 23;  // Ph Ne
      // sosNbr = 34;   // Ne Te
...
int contNeTe()
{
  ...
  if (phase == false && neutre == true && terre == true)
  {
    while (phase == false && neutre == true && terre == true)
    {
      // sosNbr = 234;  // Ph Ne Te
      // sosNbr = 23;   // Ph Ne
      sosNbr = 34; // Ne Te
...
/*************FONCTION*****cliososNbr5****/
void cliososNbr5(int sosNbr)
{
  currentMillis = millis();
  if (currentMillis - f_b_cl_Tmp < interval)
  {
    currentMillis; // mémorise la valeur de la fonction millis()
    inter = interval / 5;
    if (millis() - millisF_B_CL_5 > inter)
    {
      millisF_B_CL_5 = millis(); // mémorise la valeur de la fonction millis()
      if (flacheBleuState == LOW)
      { // inverse la variable d''état de la LED
        flacheBleuState = HIGH;
      }
  ...
  {
    f_b_cl_Tmp = currentMillis;
    compteB = compteB + 1; // cliososNbr5
  }
}
/*************FONCTION*****cliososNbr5****final*/
/*************FONCTION*****cliososNbr****/
void cliososNbr(int sosSay, int sosTmp)
{
  int say10 = sosSay;
  if (say10 == 10)
  {
  ...
  if (sosSay == 0)
  {
    digitalWrite(flacheBleu, false); // modifie l'état de la LED
    digitalWrite(flacheRouge, false);
    digitalWrite(spot, false);
    digitalWrite(sirenI, false);
    digitalWrite(sirenC, false);
      ...
      { // inverse la variable d''état de la LED
        if (millis() - f_b_cl_Say >= 300)
        {
          ...
          else
          {
            ...
            {
              digitalWrite(flacheBleu, flacheBleuState);
              digitalWrite(flacheRouge, flacheBleuState);
              digitalWrite(spot, flacheBleuState);
              digitalWrite(sirenI, flacheBleuState);
              digitalWrite(sirenC, flacheBleuState);
            }
          }
        }
      }
...
/*************FONCTION*****cliososNbr****final*/
int clioFlache(int sosNbr, int cp_cl)
{
  int curnsosNbr = sosNbr;
  if (sosNbr == 30)
  {
    sosSay = 3;
    sosTmp = 15;
    f_b_cl(sosSay); // (sosNbr == 30)
    if (cp_cl > 1)
    {
      cp_cl = 1; // sosNbr == 30
    }
